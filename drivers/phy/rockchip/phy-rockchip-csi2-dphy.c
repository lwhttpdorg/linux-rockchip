				      ...
				      struct v4l2_subdev_frame_interval *fi)
{
	...
	if (sensor)
		return v4l2_subdev_call_state_active(sensor, pad,
...
static int rockchip_csi2_dphy_get_inno_phy_hw(struct csi2_dphy *dphy)
{
	...
	for (i = 0; i < dphy->drv_data->num_inno_phy; i++) {
		...
		of_node_put(np);
		if (!plat_dev) {
			dev_dbg(dphy->dev, "dphy%d hw device is not ready\n",
				dphy->phy_index);
			return -EPROBE_DEFER;
		}
		dphy_hw = platform_get_drvdata(plat_dev);
		if (!dphy_hw) {
			put_device(&plat_dev->dev);
			dev_dbg(dphy->dev, "dphy%d hw driver is not ready\n",
				dphy->phy_index);
			return -EPROBE_DEFER;
		}
